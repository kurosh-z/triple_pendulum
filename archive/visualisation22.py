...
class Animation():
    ...
    def __init__(self, drawfnc, simdata, plotsys=[], plotinputs=[]):
        ...
        #tem. changed for Debugging xlim = (self.t[0], self.t[-1])
        xlim = (-3, 3)
        for i, idxlabel in enumerate(self.plotsys):
            idx, label = idxlabel

            try:
                # tem. changed for Debugging ylim = (min(self.xt[:,idx]), max(self.xt[:,idx]))
                ylim = (-2, 2)
            except:
                ylim = (min(self.xt), max(self.xt))

            self.set_limits(ax="ax_x{}".format(i), xlim=xlim, ylim=ylim)
            self.set_label(ax="ax_x{}".format(i), label=label)
    ...
    def get_axes(self):
        ...
        for i in range(len(sys)):
            ...
            axes["ax_x{}".format(i)].add_line(curve)
    ...
    def set_label(self, ax='ax_img', label=''):
        self.axes[ax].set_ylabel(
            label, rotation='horizontal', horizontalalignment='right')
    ...
    def show(self, t=0.0, xlim=None, ylim=None):
        ...
        for k, curve in enumerate(self.syscurves):
            ...
            self.axes["ax_x{}".format(k)].add_line(curve)
        ...
        for k, curve in enumerate(self.inputcurves):
            ...
            self.axes["ax_u{}".format(k)].add_line(curve)
    ...
    def animate(self):
        ...
        fps =around (self.nframes/(t[-1] - t[0]),0)S
        ...
        def _animate(frame):
            ...
            print(np.shape(xt))
            print(xt)
            print("frame = {f}, t = {t}, x = {x}, u = {u}".format(
                f=frame, t=t[i], x=xt[i], u=ut[i]))
            ...
            image = self.draw(xt, image=image)
            ...
            for k, curve in enumerate(self.inputcurves):
                ...
                self.axes["x_u{}".format(k)].add_line(curve)
        ...
        self.anim = animation.FuncAnimation(
            self.fig, _animate, frames=self.nframes, interval=1, blit=True)

        self.anim.save("test", fps=80, extra_args=['-vcodec', 'libx264'])

        #def save(self, fname, fps=80, dpi=200):
        #self.anim.save(filename,fps=80, extra_args=['-vcodec', 'libx264'])
        '''
