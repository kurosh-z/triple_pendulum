...
def rhs_new(x, u, uref=None, t=None, pp=None):
    q0, q1, q2, q3 = x
    u0, = u
    xd1 = q2
    xd2 = q3
...
# senkrechte Anfangs- und Endbedingungen (funktionieren bei mir)

xa = [0.0, np.pi, 0.0, 0.0]
xb = [0.0, np.pi * 0, 0.0, 0.0]

# waagerechte Anfangs- und Endbedingungen (pytra konvergiert nicht)
# if 0:
...
cs = control_sys = pytr.ControlSystem(
    rhs_new, a=0, b=2.0, xa=xa, xb=xb, ua=ua, ub=ub, **additional_parameters)
cs_ret= cs.solve()
...
#saving Trajectory to a file
# save_xtrajectory = open("xs_inv_pen", 'wb')
# pickle.dump(xs, save_xtrajectory)
# save_xtrajectory.close()
# dill.settings['recurse'] = True
# dill.settings['fmode'] = True

# with open("inv_pen2", "w") as dill_file :
#     dill.dump(cs_ret, dill_file)
